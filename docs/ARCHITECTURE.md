...
Visits store a raw-count flag and a first-seen-token-count flag separately. A form start is recorded at most once for the existing anonymous visit after the first interaction with a public feedback form. This event contains only campaign, visit, and timestamp references; it does not contain field identity, field values, drafts, or a new browser identifier. Optional opaque install tokens are limited to 256 characters, HMAC-SHA256 hashed with `KOALABYE_SECRET`, and never persisted or rendered raw. Repeated hashes may be stored as raw visits but count as unique only once. Referrers are reduced to lowercase hostnames. User agents are reduced to documented browser (`Chrome`, `Firefox`, `Safari`, `Edge`, `Other`, `Unknown`) and OS (`Windows`, `macOS`, `Linux`, `Android`, `iOS`, `Other`, `Unknown`) families; raw user agents are discarded.
...
Analytics query the existing visit, form-start, submission, answer, field, and option tables directly. There is no tracking SDK or secondary analytics store. Preset ranges are 7, 30, 90 days, and all time; finite ranges begin at UTC midnight and daily trends group ISO timestamps by UTC date. Finite periods compare against the immediately preceding period of equal length.

The funnel reports raw visits, first form interactions, submissions, and conversion rates for the selected period. Trends and field summaries honor the same filters. Field summaries parse stored JSON values: ratings show average and counts 1-5, radio and checkbox fields show option counts, percentages, and previous-period change, and textareas show only answer counts with a link to the inbox. Archived fields remain visible when historical answers reference their public ID and label snapshot.

Authorized users can narrow analytics and responses by dates and by values actually present in collected app version, extension version, platform, browser, and operating-system data. Diagnostic filters render only when their campaign collection setting is enabled. Campaign comparison is restricted to campaigns the current user can view in the same organization. Charts are inline SVG with an accessible HTML table alternative and local CSS; no chart library or external asset is used.

CSV exports can include selected identifier, timestamp, permitted diagnostic, and answer columns. Answer columns use historical field public IDs and sanitized labels. Checkbox arrays are semicolon-separated, spreadsheet formulas are neutralized, and standard CSV quoting protects commas, quotes, and newlines. JSON export version 1 preserves typed answer values and field snapshots. Both formats honor the active filters, expose only public identifiers and the boolean presence of an install-token hash, and omit diagnostics disabled by campaign settings. Export audit metadata contains format, campaign public ID, and approximate submission count, never answers.
...
Campaign settings can enable a 30, 90, 180, or 365 day retention threshold. This phase deliberately has no background scheduler. A campaign owner manually deletes visits and submissions older than the UTC cutoff in one transaction. Answers cascade with deleted submissions; form starts cascade with deleted visits; deleting a visit sets surviving submission links to `NULL`.
